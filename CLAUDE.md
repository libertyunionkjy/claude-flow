...
你当前工作在一个 Git Worktree 隔离环境中：
- **工作目录**：`/opt/shared/claude-flow/.claude-flow/worktrees/task-b007dd`
- **任务 ID**：`task-b007dd`
- **主仓库**：`/opt/shared/claude-flow`（禁止直接修改）

**强制规则**：
1. 所有文件读写操作必须限定在 `/opt/shared/claude-flow/.claude-flow/worktrees/task-b007dd` 目录内
2. 禁止使用 `/opt/shared/claude-flow` 的绝对路径操作文件
3. 使用相对路径或以 `/opt/shared/claude-flow/.claude-flow/worktrees/task-b007dd` 为前缀的绝对路径
