...
| 语言 | Python 3.10+ |
| CLI 框架 | Click >= 8.0 |
| Web 框架 | Flask >= 2.0, flask-sock >= 0.7（可选） |
| 终端 | xterm.js 5.x + PTY（Mini Task 浏览器终端） |
...
- **Mini Task 层** (`pty_manager.py`, `web/ws.py`) -- PTY 终端管理和 WebSocket 桥接
...
cf task mini -p "prompt" "标题"      # 创建 Mini Task（跳过 plan/review）
cf task mini -p "prompt" "标题" --run  # 创建并立即启动 Mini Task
cf task list                         # 查看任务列表
...
| `test_cli.py` | `cli.py` |
| `test_pty_manager.py` | `pty_manager.py` |
| `test_mini_task.py` | Mini Task 模型/API 综合测试 |
| `test_mini_task_integration.py` | Mini Task 集成测试 |
